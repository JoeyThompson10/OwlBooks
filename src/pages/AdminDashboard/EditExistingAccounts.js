...
const ChartOfAccounts = () => {
  ...
  async function handleSaveChanges(e){
    ...
    if((accountIsActive===false) && ( (accountBalance!="0")&&(accountBalance!="0.00")&&(accountBalance!="$0")&&(accountBalance!="$0.00") ) ){
      ...
      const response = await setAccountInfo(tempAccountID, accountName, accountDescription, accountNormalSide, accountCategory, accountSubcategory, accountInitialBalance, accountDebit, accountCredit, accountBalance, accountTimeCreated, "username", accountOrder, accountStatement, accountComment, true);

    }else{
      const response = await setAccountInfo(tempAccountID, accountName, accountDescription, accountNormalSide, accountCategory, accountSubcategory, accountInitialBalance, accountDebit, accountCredit, accountBalance, accountTimeCreated, "username", accountOrder, accountStatement, accountComment, accountIsActive);
      window.alert(response.message);
    }
